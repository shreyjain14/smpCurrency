...
public class CompanyManager {
    ...
    private static final Pattern COMPANY_ID_PATTERN = Pattern.compile("[a-z0-9_]{2,32}");
    ...
    public void load() {
        ...
        if (root != null) {
            for (String id : root.getKeys(false)) {
                ...
                companies.put(id.toLowerCase(Locale.ROOT), new Company(id, name, directors, totalShares));
            }
        }
    ...
    public boolean isValidCompanyId(String id) {
        return id != null && COMPANY_ID_PATTERN.matcher(id).matches();
    }
    ...
    public Company createCompany(String id, String name, UUID creator) {
        ...
        Company company = new Company(normId, name == null || name.isBlank() ? normId : name, new HashSet<>(), 0);
        company.directors().add(creator);
        companies.put(normId, company);
        save();
        return company;
    }
    ...
    public int issueShares(String companyId, UUID director, Player recipient, int amount) {
        ...
        while (remaining > 0) {
            ...
            ItemStack stack = CustomItems.createShare(company.id(), stackAmount);
            Map<Integer, ItemStack> overflow = recipient.getInventory().addItem(stack);
            // Drop overflow
            overflow.values().forEach(i -> recipient.getWorld().dropItemNaturally(recipient.getLocation(), i));
            remaining -= stackAmount;
        }
        company.totalShares(company.totalShares() + amount);
        save();
        return amount;
    }

    public List<String> listCompanyIds() { return new ArrayList<>(companies.keySet()); }
